...
import { EditEvent } from "@/interfaces/Events";
...
const EditEventModal: React.FC<EditEvent> = (props) => {
  ...
  const formik = useFormik({
    initialValues: {
      ...
      bgImg: props.events?.bgImg,
      eventDateTime: props.events?.eventDateTime
        ? new Date(props.events?.eventDateTime)
        : null,
      amount: props.events?.amount,
    },
    validationSchema: Yup.object({
      title: Yup.string().min(4, "Minimum 4 characters Required").required("Required"),
      description: Yup.string().min(4, "Minimum 4 characters Required").required("Required"),
        ...
        <form onSubmit={formik.handleSubmit} className="p-6 space-y-6 max-h-96 overflow-y-auto">
          ...
          <div className="space-y-2">
            ...
            {formik.touched.eventDateTime && formik.errors.eventDateTime && (
              <div className="text-red-300 text-sm mt-1">{formik.errors.eventDateTime}</div>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            {formik.touched.bgImg && formik.errors.bgImg && (
              <div className="text-red-300 text-sm mt-1">{formik.errors.bgImg}</div>
            )}
          </div>
